...
async function main(): Promise<void> {
  ...
  const db = getDb();
  await db.prepare("DELETE FROM onec_nomenclature_cards WHERE guid LIKE 'test-%'").run();

  console.log('\n=== bulkUpsert inserts new rows ===');
  const inserted = await onecNomenclatureRepo.bulkUpsert([
  ...
  const row = await onecNomenclatureRepo.getByGuid('test-1', SCRIPT_OWNER_ID);
  ...
  const updated = await onecNomenclatureRepo.getByGuid('test-1', SCRIPT_OWNER_ID);
  assert(updated?.name === 'Картофель новый', 'name updated');

  console.log('\n=== listItems excludes folders by default ===');
  const items = await onecNomenclatureRepo.listItems({ ownerUserId: SCRIPT_OWNER_ID,  excludeFolders: true });
  ...
  const stats = await onecNomenclatureRepo.stats(SCRIPT_OWNER_ID);
