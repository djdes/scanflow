...
async function main(): Promise<void> {
  ...
  const db = getDb();
  await db.prepare("DELETE FROM onec_nomenclature WHERE guid LIKE 'test-%'").run();
  ...
  const row = await onecNomenclatureRepo.getByGuid('test-1');
  ...
  const updated = await onecNomenclatureRepo.getByGuid('test-1');
