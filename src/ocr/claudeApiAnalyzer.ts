...
total_sum (САМОЕ ВАЖНОЕ):
- Это ВСЕГДА значение из строки итогов в самом низу таблицы. Ищи одну из фраз:
  "Всего к оплате", "Всего к оплате (9)", "Всего по накладной", "Итого", "Итого к оплате",
  "К оплате", "Итого с НДС", "Сумма к оплате". Строка итога обычно ЖИРНЫМ шрифтом.
- В строке итога ВСЕГДА ДВЕ цифры: слева — "без налога" (колонка 5), справа — "с налогом" (колонка 9). Возьми ПРАВУЮ.
- Это ОДНА единственная цифра для ВСЕГО документа (не сумма каких-то страниц). Она уже включает все позиции и все страницы — не нужно её складывать с чем-либо.
- Если документ на нескольких листах, итог напечатан ТОЛЬКО на последнем листе. На промежуточных листах цифр итога нет — если это не последний лист, total_sum: null.
- total_sum должен равняться Σ(items.total) из колонки 9 с точностью до копеек. Если расходится — ПЕРЕЧИТАЙ обе цифры: либо в позиции взял "без НДС", либо в total_sum взял "без НДС", либо пропустил позицию.
- НИКОГДА не возвращай как total_sum цифру из последней строки таблицы товаров. Это цена последнего ТОВАРА, не итог документа.
