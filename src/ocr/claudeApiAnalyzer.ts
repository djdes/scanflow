...
КРИТИЧНО про суммы (ТОРГ-12, УПД, счёт-фактура часто имеют ДВА столбца итогов):
- В таблице товаров БЕРИ КОЛОНКУ "Всего с НДС" / "Сумма с НДС" / "К оплате" — она САМАЯ ПРАВАЯ из числовых. Колонка "Сумма без НДС" стоит ЛЕВЕЕ и её НЕ использовать.
- price (цена за единицу товара): тоже бери "с НДС", если есть две колонки цены. Если указана только одна — берёшь её.
- total (сумма по позиции): ВСЕГДА "Всего с НДС" (самая правая колонка суммы). total ДОЛЖЕН РАВНЯТЬСЯ quantity × price с учётом НДС.
- total_sum (итог по документу): строка "Всего по накладной" / "Итого к оплате" / "Всего с НДС" в самом низу таблицы. Это цифра С НДС (она равна сумме всех items.total и ВКЛЮЧАЕТ vat_sum).
- Проверочная арифметика: total_sum ≈ Σ(items.total); total_sum − vat_sum ≈ сумма без НДС. Если получается иначе — ты взял "без НДС" вместо "с НДС", ПЕРЕЧИТАЙ таблицу.
