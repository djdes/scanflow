...
export const ocrCorrectionRepo = {
  async remember(supplierKey: string, fieldName: string, originalValue: unknown, correctedValue: unknown): Promise<void> {
    ...
    await getDb().prepare(`
      INSERT INTO ocr_corrections
        (supplier_key, field_name, original_hash, original_value, corrected_value)
      VALUES (?, ?, ?, ?, ?)
      ON DUPLICATE KEY UPDATE corrected_value = VALUES(corrected_value),
        times_seen = times_seen + 1, updated_at = NOW()
    `).run(supplierKey, fieldName, hash, String(originalValue ?? '').slice(0, 1024), corrected.slice(0, 1024));
  },

  async apply<T extends Record<string, unknown>>(data: T): Promise<T> {
    ...
    for (const field of fields) {
      ...
      const correction = await getDb().prepare(`
        SELECT id, corrected_value FROM ocr_corrections
         WHERE supplier_key IN (?, 'name:unknown') AND field_name = ? AND original_hash = ?
         ORDER BY supplier_key = ? DESC, times_seen DESC LIMIT 1
      `).get<{ id: number; corrected_value: string }>(supplierKey, field, hash, supplierKey);
      if (!correction) continue;
      (data as Record<string, unknown>)[field] = correction.corrected_value;
      await getDb().prepare('UPDATE ocr_corrections SET last_used_at = NOW() WHERE id = ?').run(correction.id);
    }
    ...
    for (const item of items) {
      ...
      const correction = await getDb().prepare(`
        SELECT id, corrected_value FROM ocr_corrections
         WHERE supplier_key = ? AND field_name = 'item_unit' AND original_hash = ?
         ORDER BY times_seen DESC LIMIT 1
      `).get<{ id: number; corrected_value: string }>(supplierKey, hash);
      if (!correction) continue;
