...
2. Отправьте `/scanflow-dispatch` — слэш-команда с этим же промптом лежит в
   ...
   Если команда не подхватилась (сессия открыта до её появления) — вставьте промпт ниже
   целиком одним сообщением. Claude далее работает сам:

**Промпт сессии не обновляется с диска.** Если инструкция изменилась, перезапускать сессию
не нужно — достаточно отправить `/scanflow-dispatch` новым сообщением в ту же сессию.
