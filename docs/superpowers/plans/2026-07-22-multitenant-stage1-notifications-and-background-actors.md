...
> **Статус: РЕАЛИЗОВАН** 2026-07-22. Коммиты `e45b12e`, `49403ea`, `b073fdb`, `8d31283`, `50adfe8`.
>
> Одно отклонение от плана, найденное прогоном тестов: Task 1 в исходной редакции
> требовал владельца безусловно и при его отсутствии молчал. Существующий набор
> `tests/notifications/elevated-prices.test.ts` показал, что это ломает боевой
> сценарий — накладные, положенные прямо в `inbox/`, создаются watcher'ом без
> пользовательского контекста и имеют `owner_user_id = NULL`, то есть уведомления
> по ним исчезли бы совсем. Правило уточнено: накладная **с** владельцем никогда
> не адресуется никому другому, накладная **без** владельца откатывается к
> инициатору и затем к администратору платформы. См. коммит `50adfe8`.
