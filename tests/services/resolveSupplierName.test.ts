...
describe.runIf((process.env.DB_NAME || '').includes('test'))('resolveSupplierName — directory by ИНН', () => {
  ...
  it('uses the verified directory name when ИНН matches, over the OCR name', async () => {
    ...
    await supplierRepo.create({ inn: '7722316694', name: CANON, bank_bic: '044525225', verified: 1 });
    expect(await resolveSupplierName('ООО ТД НИЖЕГ ХЛЕБ', '7722316694')).toBe(CANON);
  });

  it('fills the name from the directory even when OCR missed the name entirely', async () => {
    await supplierRepo.create({ inn: '7724400648', name: 'ООО "ГРОСС ФУД"', bank_bic: '044525225', verified: 1 });
    expect(await resolveSupplierName('', '7724400648')).toBe('ООО "ГРОСС ФУД"');
    expect(await resolveSupplierName(null, '7724400648')).toBe('ООО "ГРОСС ФУД"');
  });

  it('ignores an UNVERIFIED directory card (photo-extract names may be garbled)', async () => {
    await supplierRepo.create({ inn: '5018202085', name: 'ООО ГРЯЗНЫЙ OCR', bank_bic: '044525225', verified: 0 });
    const out = await resolveSupplierName('ООО "Чистый Поставщик"', '5018202085');
  ...
  it('falls back to prior-invoice canonical spelling when no directory card exists', async () => {
    ...
    expect(await resolveSupplierName('ООО "ВЕСЕЛОФФ и ГКОМПАНИ"', '5018202085')).toBe(A);
  });

  it('returns undefined when there is neither a directory card nor a raw name', async () => {
    expect(await resolveSupplierName('', '9999999999')).toBeUndefined();
    expect(await resolveSupplierName(null, null)).toBeUndefined();
  });
});
